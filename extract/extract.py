...
def load_version(args):
    ...
    if args.version == '2016.1':
        ...
        for (study_id, subst_id, sex, route, species, exposure) in results:
            ...
            tmp_table.append(
                [study_id, subst_id, sex, route, species, exposure])
        ...
        find_df = pd.DataFrame(tmp_table, columns=['study_id',
                                                   'relevance',
...
def filter_study(args, study_df):
    ...
    if args.route:
        df = df[df.normalised_administration_route.str.lower().isin(
            [x.lower() for x in args.route])]
...
def expand(df, args):
    ...
    for organ in args.organ:
        ...
        all_related_organs = all_related_organs.union(
            related_organs.child_term)
    ...
    if args.observation is None:
        ...
        for observation in args.observation:
            ...
            related_observation = onto_df[(onto_df.parent_term == observation)
                                          & (onto_df.ontology == 'histopathology')]
            all_related_observation = all_related_observation.union(
                related_observation.child_term)
            ...
            observation_dict.update(
                {n: organ for n in all_related_observation})
            all_observations = all_organs.union(
                all_related_observation, all_observation)

        findings_out = df[df['observation_normalised'].isin(all_organs)]
        findings_out.loc[:, 'observation_normalised'] = df['observation_normalised'].map(
            organs_dict)
...
def run(args):
    ...
    relevant_find = find_df[find_df.study_id.isin(
        relevant_studies_df.study_id)]
    ...
    count_df = relevant_find.groupby(
        ('subst_id')).study_id.nunique().to_frame().reset_index()
    # Get the global dose range per substance
    range_df = relevant_find[relevant_find.dose > 0]
    range_df = range_df.groupby(('subst_id')).dose.apply(
    ...
    sys.stderr.write(
        'Expand based on anatomic and morphological change ontologies\n')
    ...
    filtered_find.organ_normalised = filtered_find.organ_normalised.fillna(
        'NA')
    filtered_find.observation_normalised = filtered_find.observation_normalised.fillna(
        'NA')
    filtered_find['finding'] = filtered_find.apply(
    ...
    # keeping the minimum dose
    # for each substance/finding instance
    group_df = filtered_find.groupby(
        ('subst_id', 'finding')).min().add_prefix('min_').reset_index()

    #######################################
    # Pivot so that each finding is a row #
    #######################################
    # Quantitative
    pivotted_df = group_df.pivot_table(
        index='subst_id', columns='finding', values='min_dose').reset_index()
...
def main():
    ...
    if args.observation:
        ...
        for obs in args.observation:
            right_case_obs = list(
                onto_df[onto_df['parent_term'].str.lower() == obs].parent_term.unique())
            if len(right_case_obs) == 0:
                sys.stderr.write(
                    'The observation %s is not found in the database.\n' % obs)
            else:
    ...
    for org in args.organ:
        right_case_obs = list(
            onto_df[onto_df['parent_term'].str.lower() == org].parent_term.unique())
        if len(right_case_obs) == 0:
            sys.stderr.write(
                'The organ %s is not found in the database.\n' % org)
        else:
